...
func _provide_input(_input):
	...
	if Input.is_action_pressed("ui_left"):		_input.steering -= 0.01 +  ( get_parent().brakes / 100.0 )
	elif Input.is_action_pressed("ui_right"):		_input.steering += 0.01 + ( get_parent().brakes / 100.0 )
